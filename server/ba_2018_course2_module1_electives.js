...
app.get('/polls/ba-2018-course2-module1-electives', function(req, res) {
  res.render('ba-2018-course2-module1-electives', {user: req.user})
});

app.post('/polls/ba-2018-course2-module1-electives', function(req, res) {
  Session.findOne({
...
function parseSession (sess, req, transporter){
  ...
  sess.polls.ba_2018_course2_module1_electives.time = now.toLocaleString('en-US', {timeZone: 'Asia/Yekaterinburg'});
  var keyNames = Object.keys(req.body);
  keyNames.forEach((el)=>{
    console.log(req.body[el]);

      sess.polls.ba_2018_course2_module1_electives[el] = req.body[el];
});
