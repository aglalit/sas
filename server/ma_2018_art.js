...
app.get('/polls/ma-2018-art', function(req, res) {
  res.render('ma-2018-art', {user: req.user})
});

app.post('/polls/ma-2018-art', function(req, res) {
  Session.findOne({
...
function parseSession (sess, req, transporter){
  ...
  sess.polls.ma_2018_art.time = now.toLocaleString('en-US', {timeZone: 'Asia/Yekaterinburg'});
  var keyNames = Object.keys(req.body);
  keyNames.forEach((el)=>{
    console.log(req.body[el]);

      sess.polls.ma_2018_art[el] = req.body[el];
});
  ...
  let mailOptions = {
    ...
    subject: 'Wolf — Feedback', // Subject line
