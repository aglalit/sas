module.exports = function(app, Session, transporter, officeTransporter, isLoggedIn, User, logger) {
  ...
  app.post('/polls/ba-2019-year2-module8-electives', function(req, res) {
    User.findOne({
      '_id': req.user._id
    }, function(err, user) {
      if (err)
    ...
    function parseSession(sess, req, transporter) {
      ...
      let mailOptions = {
        ...
        subject: 'Elective year 2/1 — feedback', // Subject line
      ...
      sess.polls.ba_2019_year2_module8_electives = JSON.stringify(req.body);
