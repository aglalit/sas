module.exports = function(app, Session, transporter, isLoggedIn, User, logger) {
  ...
  app.get('/polls/ba-2019-year1-module4-history', isLoggedIn, function(req, res) {
    res.render('ba-2019-year1-module4-history', {
      user: req.user
    })
  });

  app.post('/polls/ba-2019-year1-module4-history', function(req, res) {
    User.findOne({
      '_id': req.user._id
    }, function(err, user) {
      if (err)
    ...
    function parseSession(sess, req, transporter) {
      ...
      sess.polls.ba_2019_year1_module4_history = JSON.stringify(req.body);
