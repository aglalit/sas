module.exports = function(app, Session, transporter, isLoggedIn, User){
  ...
  app.post('/polls/majors', function(req, res) {
    User.findOne({
      ...
      if (user) {
        user.polls.majors_2021.major = req.body["major"];
        user.polls.majors_2021.minor = req.body["minor"];

        var now = new Date();
        user.polls.majors_2021.time = now.toLocaleString('en-US', {timeZone: 'Asia/Yekaterinburg'});
    ...
    req.flash('info', `Your choice is submitted (${req.body["major"]}). In case of a mistake, you can make your choice again. Thanks for participation ( ͡° ͜ʖ ͡°)`);
    let emailBody = '';
