...
app.get('/auth/google/callback', passport.authenticate('google', {
  successRedirect: '/polls/2nd-module-surendra',
...
app.get('/polls/calvert-forum', function(req, res) {
...
app.post('/polls/calvert-forum', function(req, res) {
  Session.findOne({
    ...
    if (session) {
      ...
      session.polls.Calvert.language = 'en';
      session.polls.Calvert.time = now.toLocaleString('en-US', {timeZone: 'Asia/Yekaterinburg'});
      session.polls.Calvert.LECTURES["Как хорошо было организовано информационное сопровождение Форума"] = parseInt(req.body["Как хорошо было организовано информационное сопровождение Форума"]);
      session.polls.Calvert.LECTURES["Оцените сложность содержания тематических сессий Форума"] = parseInt(req.body["Оцените сложность содержания тематических сессий Форума"]);
      session.polls.Calvert.LECTURES["Насколько новым для вас было содержание тематических сессий Форума"] = parseInt(req.body["Насколько новым для вас было содержание тематических сессий Форума"]);
      session.polls.Calvert.LECTURES["Насколько изменились ваше мышление, знания и умения под воздействием Форума"] = parseInt(req.body["Насколько изменились ваше мышление, знания и умения под воздействием Форума"]);
      session.polls.Calvert.LECTURES["Что вам больше всего понравилось на Форуме?"] = parseInt(req.body["Что вам больше всего понравилось на Форуме?"]);
      session.polls.Calvert.LECTURES["Что вам больше всего не понравилось на Форуме?"] = parseInt(req.body["Что вам больше всего не понравилось на Форуме?"]);

      session.polls.Calvert.SEMINARS["Что именно для вас было непонятным и самым сложным в установке на групповую работу?"] = req.body["Что именно для вас было непонятным и самым сложным в установке на групповую работу?"];
      session.polls.Calvert.SEMINARS["Оцените, насколько понятной для вас была установка на групповую работу"] = req.body["Оцените, насколько понятной для вас была установка на групповую работу"];
      session.polls.Calvert.SEMINARS["Укажите имя и фамилию куратора, организовывавшего групповую работу"] = req.body["Укажите имя и фамилию куратора, организовывавшего групповую работу"];
      session.polls.Calvert.SEMINARS["Насколько хорошо куратор организует интерактивную коммуникацию в группе?"] = parseInt(req.body["Насколько хорошо куратор организует интерактивную коммуникацию в группе?"]);
      session.polls.Calvert.SEMINARS["Сколько раз в течение групповой работы вы (в среднем) высказывались вслух?"] = parseInt(req.body["Сколько раз в течение групповой работы вы (в среднем) высказывались вслух?"]);
      session.polls.Calvert.SEMINARS["Сколько процентов обязательных текстов по этой части курса вы успевали прочесть"] = parseInt(req.body["Сколько процентов обязательных текстов по этой части курса вы успевали прочесть"]);
      session.polls.Calvert.SEMINARS["Что вам больше всего понравилось в групповой работе?"] = parseInt(req.body["Что вам больше всего понравилось в групповой работе?"]);
      session.polls.Calvert.SEMINARS["Что вам больше всего не понравилось в групповой работе?"] = parseInt(req.body["Что вам больше всего не понравилось в групповой работе?"]);
      session.polls.Calvert.SEMINARS["Что для вас было самым сложным в групповой работе?"] = parseInt(req.body["Что для вас было самым сложным в групповой работе?"]);
      session.polls.Calvert.SEMINARS["Что вам больше всего понравилось на пленаре?"] = parseInt(req.body["Что вам больше всего понравилось на пленаре?"]);
      session.polls.Calvert.SEMINARS["Что вам больше всего не понравилось на пленаре?"] = parseInt(req.body["Что вам больше всего не понравилось на пленаре?"]);
      session.polls.Calvert.SEMINARS["Что для вас было самым сложным на пленаре?"] = parseInt(req.body["Что для вас было самым сложным на пленаре?"]);
      session.polls.Calvert.SEMINARS["Как бы вы порекомендовали улучшить этот курс"] = parseInt(req.body["Как бы вы порекомендовали улучшить этот курс"]);
      ...
      var now = new Date();
      newSession.session_id = req.session.id;
      newSession.polls.Calvert.language = 'en';
      newSession.polls.Calvert.time = now.toLocaleString('en-US', {timeZone: 'Asia/Yekaterinburg'});
      newSession.polls.Calvert.LECTURES["Как хорошо было организовано информационное сопровождение Форума"] = parseInt(req.body["Как хорошо было организовано информационное сопровождение Форума"]);
      newSession.polls.Calvert.LECTURES["Оцените сложность содержания тематических сессий Форума"] = parseInt(req.body["Оцените сложность содержания тематических сессий Форума"]);
      newSession.polls.Calvert.LECTURES["Насколько новым для вас было содержание тематических сессий Форума"] = parseInt(req.body["Насколько новым для вас было содержание тематических сессий Форума"]);
      newSession.polls.Calvert.LECTURES["Насколько изменились ваше мышление, знания и умения под воздействием Форума"] = parseInt(req.body["Насколько изменились ваше мышление, знания и умения под воздействием Форума"]);
      newSession.polls.Calvert.LECTURES["Что вам больше всего понравилось на Форуме?"] = parseInt(req.body["Что вам больше всего понравилось на Форуме?"]);
      newSession.polls.Calvert.LECTURES["Что вам больше всего не понравилось на Форуме?"] = parseInt(req.body["Что вам больше всего не понравилось на Форуме?"]);

      newSession.polls.Calvert.SEMINARS["Что именно для вас было непонятным и самым сложным в установке на групповую работу?"] = req.body["Что именно для вас было непонятным и самым сложным в установке на групповую работу?"];
      newSession.polls.Calvert.SEMINARS["Оцените, насколько понятной для вас была установка на групповую работу"] = req.body["Оцените, насколько понятной для вас была установка на групповую работу"];
      newSession.polls.Calvert.SEMINARS["Укажите имя и фамилию куратора, организовывавшего групповую работу"] = req.body["Укажите имя и фамилию куратора, организовывавшего групповую работу"];
      newSession.polls.Calvert.SEMINARS["Насколько хорошо куратор организует интерактивную коммуникацию в группе?"] = parseInt(req.body["Насколько хорошо куратор организует интерактивную коммуникацию в группе?"]);
      newSession.polls.Calvert.SEMINARS["Сколько раз в течение групповой работы вы (в среднем) высказывались вслух?"] = parseInt(req.body["Сколько раз в течение групповой работы вы (в среднем) высказывались вслух?"]);
      newSession.polls.Calvert.SEMINARS["Сколько процентов обязательных текстов по этой части курса вы успевали прочесть"] = parseInt(req.body["Сколько процентов обязательных текстов по этой части курса вы успевали прочесть"]);
      newSession.polls.Calvert.SEMINARS["Что вам больше всего понравилось в групповой работе?"] = parseInt(req.body["Что вам больше всего понравилось в групповой работе?"]);
      newSession.polls.Calvert.SEMINARS["Что вам больше всего не понравилось в групповой работе?"] = parseInt(req.body["Что вам больше всего не понравилось в групповой работе?"]);
      newSession.polls.Calvert.SEMINARS["Что для вас было самым сложным в групповой работе?"] = parseInt(req.body["Что для вас было самым сложным в групповой работе?"]);
      newSession.polls.Calvert.SEMINARS["Что вам больше всего понравилось на пленаре?"] = parseInt(req.body["Что вам больше всего понравилось на пленаре?"]);
      newSession.polls.Calvert.SEMINARS["Что вам больше всего не понравилось на пленаре?"] = parseInt(req.body["Что вам больше всего не понравилось на пленаре?"]);
      newSession.polls.Calvert.SEMINARS["Что для вас было самым сложным на пленаре?"] = parseInt(req.body["Что для вас было самым сложным на пленаре?"]);
      newSession.polls.Calvert.SEMINARS["Как бы вы порекомендовали улучшить этот курс"] = parseInt(req.body["Как бы вы порекомендовали улучшить этот курс"]);
